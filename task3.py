...
def Ui(l):
    """
    Описание функции Ui:
    Заправшивает и обрабатывает пользовательский ввод
    выполняет поиск по базе сбитых часов и выводит восстановленное время если нашлась станция с номером введенным пользователем
    :param l: база сбитых часов
    :return: продолжает ли работу пользователь (булевое значение)
    """
...
def main():
    ...
    читает файл базы данных и передает его фукции Ui, запускает фукцию Ui в бесконечном до завершения работы пользователем цикле
